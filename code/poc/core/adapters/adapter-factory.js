...
export class AdapterFactory {
    ...
    createRebasedAdapter(config) {
        ...
        if (config.useProductionAdapter === true) {
            ...
            if (ProductionRebasedAdapter.length > 0) {
                return new ProductionRebasedAdapter(productionConfig);
            }
            else {
        ...
        if (config.account || config.sponsorWallet || config.contractAddresses) {
            ...
            try {
                const rebasedAdapter = new RebasedAdapter(config.nodeUrl, config.apiKey, config.seed);
                return rebasedAdapter;
            }
            catch (error) {
    ...
    createEVMAdapter(config) {
        ...
        try {
            ...
            const evmAdapter = new EVMAdapter(config.rpcUrl, config.contractAddresses, config.privateKey, config.chainId);
            return evmAdapter;
        }
        catch (error) {
