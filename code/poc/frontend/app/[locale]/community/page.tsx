...
function CompactUserCard({ user, onFollow, onUnfollow, isLoading, t }: CompactUserCardProps) {
  ...
  return (
    <div 
      ...
      className="flex-shrink-0 w-[140px] bg-white rounded-xl border border-stone-200 p-3 cursor-pointer hover:shadow-md hover:border-[#FF6B5B]/30 transition-all duration-200"
    >
      {/* Avatar */}
      <div className="flex justify-center mb-2">
        <div className="w-10 h-10 rounded-full overflow-hidden bg-gradient-to-br from-[#FF6B5B] to-[#FF8F85] flex items-center justify-center">
          {user.avatar_url ? (
            <Image
              src={user.avatar_url}
      ...
      <button
        ...
        className={`w-full py-1.5 px-2 rounded-lg text-xs font-medium transition-all duration-200 ${
          isFollowing
            ...
            : 'bg-[#FF6B5B] text-white hover:bg-[#E55A4A]'
...
function UserCard({ user, onFollow, onUnfollow, isLoading, t }: UserCardProps) {
  ...
  return (
    <div 
      onClick={handleCardClick}
      className="bg-white rounded-xl border border-stone-200 p-4 cursor-pointer hover:shadow-md hover:border-[#FF6B5B]/30 transition-all duration-200"
    >
      <div className="flex items-start gap-4">
        {/* Avatar */}
        <div className="w-12 h-12 rounded-full overflow-hidden bg-gradient-to-br from-[#FF6B5B] to-[#FF8F85] flex items-center justify-center flex-shrink-0">
          {user.avatar_url ? (
            <Image
              src={user.avatar_url}
              alt={user.display_name || t('user.anonymous')}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {user.verification_status === 'verified' && (
              <span className="text-[#FF6B5B]" title={t('user.verified')}>✓</span>
            )}
          </div>
...
function SearchResultItem({ user, onFollow, onUnfollow, onClick, t }: SearchResultItemProps) {
  ...
  return (
    ...
    >
      {/* Avatar */}
      <div className="w-10 h-10 rounded-full overflow-hidden bg-gradient-to-br from-[#FF6B5B] to-[#FF8F85] flex items-center justify-center flex-shrink-0">
        {user.avatar_url ? (
          <Image
            src={user.avatar_url}
            alt={user.display_name || t('user.anonymous')}
...
function ComingSoonCard({ icon, title, description, badgeText }: ComingSoonCardProps) {
  return (
    <div className="bg-gradient-to-br from-stone-50 to-stone-100 rounded-xl border border-stone-200 border-dashed p-6 opacity-75">
      <div className="flex items-start gap-4">
        <div className="w-12 h-12 rounded-xl bg-white border border-stone-200 flex items-center justify-center text-2xl">
          {icon}
        </div>
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-[#2D2D2D]">{title}</h3>
            <span className="px-2 py-0.5 bg-[#FF6B5B]/10 text-[#FF6B5B] text-xs font-medium rounded-full">
              {badgeText}
            </span>
          </div>
  ...
  return (
    <div className="relative">
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-lg font-semibold text-[#2D2D2D]">{t('discover.title')}</h2>
        <button 
          onClick={onSeeAll}
          className="text-sm text-[#FF6B5B] hover:text-[#E55A4A] font-medium transition-colors"
        >
          {t('discover.seeAll')}
        </button>
      </div>
      ...
      <div className="relative">
        ...
        >
          ...
          {users.length === 0 && !isLoading && (
            <div className="flex-shrink-0 w-full py-8 text-center text-stone-400">
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F0]">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        {/* Page Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-[#2D2D2D]">{t('page.title')}</h1>
        ...
        <div className="relative mb-6">
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-3 pl-10 bg-white border border-stone-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#FF6B5B]/20 focus:border-[#FF6B5B] transition-all"
            />
            <svg 
          ...
          {showSearchDropdown && (
            <div 
              ...
              className="absolute top-full left-0 right-0 mt-2 bg-white border border-stone-200 rounded-xl shadow-lg z-50 max-h-80 overflow-y-auto"
            >
              {isSearching ? (
        ...
        {activeTab === 'discover' ? (
            ...
            {!showAllDiscover ? (
              <>
                ...
                <div className="space-y-4 mt-8">
                  <h2 className="text-lg font-semibold text-[#2D2D2D]">{t('comingSoon.title')}</h2>
              ...
              <>
                ...
                <div className="space-y-3">
                  {isLoadingDiscover ? (
                    <div className="py-12 text-center text-stone-400">{t('discover.loading')}</div>
                    ...
                    <div className="py-12 text-center">
                      <p className="text-stone-400">{t('discover.empty.title')}</p>
                      <p className="text-stone-400 text-sm mt-1">{t('discover.empty.subtitle')}</p>
                    </div>
                  )}
                </div>
              </>
          ...
          <div className="space-y-3">
            {!isAuthenticated ? (
              <div className="py-12 text-center">
                <p className="text-stone-500 mb-4">{t('following.signInRequired.message')}</p>
                <Link 
                  href="/login"
                  className="inline-block px-6 py-2 bg-[#FF6B5B] text-white rounded-lg hover:bg-[#E55A4A] transition-colors"
                >
                  {t('actions.signIn')}
                </Link>
              </div>
            ) : isLoadingFollowing ? (
              <div className="py-12 text-center text-stone-400">{t('following.loading')}</div>
              ...
              <div className="py-12 text-center">
                <p className="text-stone-400">{t('following.empty.title')}</p>
                <button
                  onClick={() => setActiveTab('discover')}
                  className="mt-4 px-6 py-2 bg-[#FF6B5B] text-white rounded-lg hover:bg-[#E55A4A] transition-colors"
                >
