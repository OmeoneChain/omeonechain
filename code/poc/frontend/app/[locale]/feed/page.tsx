...
const MainFeed: React.FC = () => {
  ...
  const transformMixedFeed = useCallback((feedData: any[]): FeedItem[] => {
    return feedData.map(item => {
      // Handle reshares
      // FIX: The backend's formatFeedItem() now returns reshares with the SAME
      // pre-formatted structure as recommendations (location, author, engagement,
      // photos, aspects, etc.). Previously this handler tried to read from raw
      // database fields (item.restaurants, item.users) which no longer exist in
      // the formatted response, causing "Unknown Restaurant" / "Unknown User".
      // Now we use item.location, item.author, item.engagement directly — same
      // as the recommendation handler below.
