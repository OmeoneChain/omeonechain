  ...
  if (comments.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500 text-sm">{t('comments.empty.title')}</p>
        <p className="text-gray-400 text-xs mt-1">{t('comments.empty.description')}</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-200">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-gray-500">{t('comments.sort.label')}</span>
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as SortOption)}
            className="text-sm border border-gray-300 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-coral focus:border-coral transition-all"
          >
      ...
      <div className="space-y-4">
        {sortedComments.map((comment) => (
          <CommentItem
            ...
            isRecommendationAuthor={isRecommendationAuthor}
